...
# neg的意思是negative，也就是说这个prompt会让reviewer倾向于拒绝这篇paper
reviewer_system_prompt_neg = (
        reviewer_system_prompt_base
        + "If a paper is bad or you are unsure, give it bad scores and reject it."
)

# pos的意思是positive，也就是说这个prompt会让reviewer倾向于接受这篇paper
...
# 这个neurips的全称是Neural Information Processing Systems，这个prompt会不会有点太长了？！
neurips_form = (
        """
    ## Review Form
...
def perform_review(
    ...
    """
    Perform a review of a paper using the LLM model.
    text: 被review的paper的文本
    model: LLM模型
    client: LLM模型的client
    num_reflections: 反思的次数
    num_fs_examples: 使用的fewshot例子的数量
    num_reviews_ensemble: ensemble的数量
    temperature: LLM的温度
    msg_history: 消息历史
    return_msg_history: 是否返回消息历史
    reviewer_system_prompt: reviewer的system prompt
    review_instruction_form: review的instruction form
    """
...
def load_paper(pdf_path, num_pages=None, min_size=100):
    """
    先尝试使用pymupdf4llm，然后尝试pymupdf，最后尝试pypdf
    """
    try:
    ...
    print(f'load_paper, path: {pdf_path}, text.length: {len(text)}')
    return text
